...
class AnalyzeEpisodeActionDistribution(GraphAnalyzer):
    ...
    def add_plots(self):
        ...
        action_names = self.action_space.get_all_action_names_for_x_axis()
        this_episode_counts = get_counts_as_percentage(np.array(episode.action_frequency))
        ...
        if show_filtered and show_all:
            filtered_episodes_counts = get_data_for_episodes(self.filtered_episodes)
            all_episodes_counts = get_data_for_episodes(self.all_episodes)
