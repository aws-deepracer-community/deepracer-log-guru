...
class ActionSpaceFilterDialog(Dialog):
    def __init__(self, parent):
        ...
        for action in self.action_space_filter.action_space:
            if action is not None:
                self.action_choices.append(BooleanVar(value=self.action_space_filter.action_on[action.index]))
    ...
    def body(self, master):
        ...
        col = 0
        row = 0
        for i, action in enumerate(self.action_space_filter.action_space):
            if action is not None:
                Checkbutton(
    ...
    def apply(self):
        for i, action in enumerate(self.action_space_filter.action_space):
            if action is not None:
                self.action_space_filter.action_on[action.index] = self.action_choices[i].get()
